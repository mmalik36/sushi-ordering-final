...
async function main() {
    ...
    try {
        ...
        const productsCollection = client.db("sushi-ordering").collection("products");
        const categoriesCollection = client.db("sushi-ordering").collection("categories");

        let categories = ['soup', 'salad', 'Appetizer from kitchen', 'Appetizer from sushi bar', 'Signature Roll'].map((category) => { return { name: category } });
